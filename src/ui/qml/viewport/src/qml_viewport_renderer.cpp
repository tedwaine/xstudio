...
void QMLViewportRenderer::frameSwapped() {
    if (viewport_renderer_) viewport_renderer_->framebuffer_swapped(utility::clock::now());
}
...
void QMLViewportRenderer::make_xstudio_viewport() {
    ...
    if (viewport_renderer_) delete viewport_renderer_;
    ...
    ui::viewport::ViewportRendererPtr gl_renderer;
    {
        ...
        if (gl_renderers.find(m_window) != gl_renderers.end()) {
            ...
            gl_renderer = ui::viewport::ViewportRendererPtr(new opengl::OpenGLViewportRenderer(false));
...
void QMLViewportRenderer::set_playhead(caf::actor playhead) {
    if (viewport_renderer_) viewport_renderer_->set_playhead(playhead);
}

void QMLViewportRenderer::reset() { if (viewport_renderer_) viewport_renderer_->reset(); }
...
QRectF QMLViewportRenderer::imageBoundsInViewportPixels() const {
    Imath::Box2f box = viewport_renderer_ ? viewport_renderer_->image_bounds_in_viewport_pixels() : Imath::Box2f();
...
Imath::V2f QMLViewportRenderer::imageCoordsToViewport(const int x, const int y) {
    return viewport_renderer_ ? viewport_renderer_->image_coordinate_to_viewport_coordinate(x, y) : Imath::V2f();
}
...
void QMLViewportRenderer::setScreenInfos(
    ...
    double refresh_rate) {
    if (viewport_renderer_) viewport_renderer_->set_screen_infos(
        name.toStdString(),
        model.toStdString(),

         ...
         [=](active_range_atom, const FrameRange &fr) -> JsonStore {
             ...
             if (not jsn.is_null()) {
                 // send(event_group_, event_atom_v, item_atom_v, jsn, false);
                   ...
                   anon_send(history, history::log_atom_v, micros, jsn);
                #elif 
         ...
         [=](event_atom, item_atom, const JsonStore &update, const bool hidden) {
             if (base_.item().update(update)) {
                 ...
                 if (not more.is_null()) {
                     ...
                     if (not hidden)
                     {
                            ...
                            auto micros = std::chrono::duration_cast<std::chrono::microseconds>(tp.time_since_epoch()).count();
                            //using nano_sys = std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds>;
                            anon_send(history, history::log_atom_v, micros, more);
                        #else
            ...
            // send(event_group_, event_atom_v, item_atom_v, update, hidden);
             if (not hidden)
             {
                    ...
                    auto tp = sysclock::now();
                    auto micros = std::chrono::duration_cast<std::chrono::microseconds>(tp.time_since_epoch()).count();
                    anon_send(history, history::log_atom_v, micros, update);
                #else
                 ...
                 .await(
                     [=](const Item &item) mutable {
                         ...
                         // send(event_group_, event_atom_v, item_atom_v, changes, false);
                            ...
                            auto micros = std::chrono::duration_cast<std::chrono::microseconds>(tp.time_since_epoch()).count();
                            //using nano_sys = std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds>;
                            anon_send(history, history::log_atom_v, micros, changes);
                        #else

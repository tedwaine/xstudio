...
void py_link(py::module_ &m) {
    py::class_<caf::python::py_context>(m, "Link")
        ...
        .def(
            ...
            "Starts an indefinite loop listening for messages from xstudio and running Python message callbacks where they have been set. See Module base class.")
        .def(
            "add_message_callback",
            &caf::python::py_context::py_add_message_callback,
            "Add a python callback function, called every time the given Actor's event group generates a message. ")
        .def(

...
utility::JsonStore SnapshotMenuModel::toMenuModelItemData(FileSystemItem *item) {
    ...
    if (item->type() == utility::FSIT_FILE) {
        ...
        if (item != &items_) {
            ...
            menu_item_data["children"].push_back(reveal_folder_item);
    ...
    try {
        ...
        if (not value.isNull()) {
            ...
            if (jsn.is_array()) {
                ...
                while (it != jsn.end()) {
                    try {
                        auto uri = caf::make_uri(it->at("path").get<std::string>());
                        if(uri) {
                            auto path = uri_to_posix_path(*uri);
                            if(not fs::exists(path)) {
                                spdlog::info("Remove missing snapshot {} {}", it->at("name").get<std::string>(), path);
                                it = jsn.erase(it);
                            }
                            else
                                it++;
